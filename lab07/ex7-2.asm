    ...
    ORG 0050H
START:
	MOV	R3,#0FFH  		//將FFH的值丟到R3暫存器之中
	MOV	R4,#0FFH     	//將FFH的值丟到R4暫存器之中
	MOV	R2,#0  			//將0的值丟到R2暫存器之中
ROW1:
...
SHOW:
	...
	MOV 	DPTR,#TABLE1//將TABLE1裡面存的數值丟給DPTR暫存器
	MOV	A,R1   			//將R1的值丟到A之中
	ADD	A,R0  			//將R0的值和A相加
...
PRINT:
	MOV	P2,R3       	//將R3的值丟到PORT2之中
	CALL	DELAY   	//跳到Delay的副程式執行程式，完成後回下行繼續執行
	MOV	P2,R4 	    	//將R4的值丟到PORT2之中
	CALL	DELAY   	//跳到Delay的副程式執行程式，完成後回下行繼續執行
	INC		R2 	 		//對R2的數值加1
	CJNE	R2,#50,PRINT//判斷R2是否不等於50，如果不等於50就跳      
...
DELAY2:
	DJNZ	R7,DELAY2  	//R7的數值先減1，然後檢察R7是否為0，如果是的話就跑下一行，如果不為0，就跳到DELAY3執行
